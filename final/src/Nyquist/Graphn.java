...
public class Graphn extends JPanel
{
	 ...
	 int  div;
	  int screenX = 1400;
	  int screenY = 900;
	  int maxX = screenX/2-30;
	  int maxY = screenY/2 -70;
	  ...
	  public Graphn(Coordinate[] points, boolean b)
	{
		LINE = b;
		this.points = points;
		...
		if(scaleX > scaleY)  
			...
			scale1 = scaleY;
		scale1 += Math.pow(5, (int)Math.log10(scale1));
		if(scale1<0)
			scale1=1;
		if(scale1>1000)
		{
	  ...
	  this.plot(g,points);
			...
			private   void plot(Graphics g,Coordinate []points)
		{
				...
				private   void line(Graphics g,Coordinate []points)
			{
				...
				int X1= (int)((points[0].x*10*div)/(scale));
				int Y1 = (int)((points[0].y*10*div)/(scale));
				
					
				
				int y= (int)(((Math.abs(points[0].y))*10*div)/(scale));
				int x = X1-y;
				int startAngle;
				if(points[0].y>0)
				 startAngle = 270;
				...
				startAngle = 90;
				//System.out.println("x  "+x+"y "+y+" startAngle "+startAngle);
			
				g.drawArc(x,-y,2*y,2*y,(int)startAngle, -180);
